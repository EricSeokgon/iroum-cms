...
class UserControllerTest {
    ...
    // 메소드 레벨 @PreAuthorize("hasAnyRole('SUPER_ADMIN','DEPT_ADMIN')") 정책 검증
    // (AUTHZ-MATRIX-001 AC-AM-002-9/10 IT 레이어와 분리: 슬라이스 vs SpringBootTest)
    // ──────────────────────────────────────────────────────────────

    @Test
    @DisplayName("AC-COV-001-1 — GET /api/v1/users 인증 없이 접근 시 403 Forbidden (@WebMvcTest 한계)")
    ...
    @DisplayName("AC-COV-001-2 — GET /api/v1/users 권한 부족 시 403 Forbidden")
    void list_returns403_withInsufficientAuthority() throws Exception {
        // given: WRONG_AUTHORITY는 SUPER_ADMIN/DEPT_ADMIN 정책 미충족
