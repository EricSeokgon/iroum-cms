...
class SafetyProfileControllerTest {
    ...
    //
    // SafetyProfileController는 클래스/메소드 레벨 @PreAuthorize 어노테이션이 없으며,
    // 운영 환경에서는 SecurityConfig의 HTTP 레벨 정책(.anyRequest().authenticated())로
    // /api/v1/safety/** 경로 인증만 강제된다. 권한(role/authority)별 차등 통제는 없다.
    //
    // 또한 일부 endpoint는 @AuthenticationPrincipal Long companyId 인자를 사용하므로
