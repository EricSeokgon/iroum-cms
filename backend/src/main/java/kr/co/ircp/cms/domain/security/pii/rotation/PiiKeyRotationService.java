 ...
 * <p>페이징 전략:
 * <ul>
 *   <li>재암호화가 끝난 row 는 email_key_version=activeVersion 으로 갱신되어
 *       다음 SELECT 의 WHERE 조건(email_key_version != activeVersion)에서 자연스럽게 제외된다.</li>
 *   <li>따라서 lastId 페이징 인자는 무한 루프 방지용 안전장치다 — 같은 row 를 두 번 다루는 것을 차단한다.</li>
 * </ul>
 */
...
public class PiiKeyRotationService {
     ...
     *
     * <p>호출자는 다음 청크의 시작 위치를 다음 호출에 전달해야 하며,
     * 청크 내 row id 들 중 최댓값은 내부 페이징 안전장치로만 사용된다 — 본 메서드는 단순히 row 수를 반환한다.
     *
     * @param activeVersion 현재 활성 키 버전 (이 값과 다른 row 만 처리)
     * @param lastId 직전 청크의 마지막 id (첫 호출 시 0)
     * @param batchSize 본 청크에서 가져올 최대 row 수
     * @return 본 청크에서 재암호화된 row 수
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public int rotateChunk(int activeVersion, long lastId, int batchSize) {
        ...
        log.debug("PII 키 회전 청크 완료 — activeVersion={}, processed={}", activeVersion, processed);
        return processed;
    }
    ...
    public int rotatePendingAll() {
        ...
        // 재암호화된 row 는 email_key_version=activeVersion 으로 갱신되어
        // 다음 호출의 WHERE 조건에서 자연스럽게 제외된다 → lastId 는 0 으로 충분.
        try {
            while (true) {
                int processed = rotateChunk(activeVersion, 0L, batchSize);
                if (processed == 0) {
