...
- 2026-06-18 (v1.0.0): 초안 작성. Phase 0.5 research.md 기반. notification_template 스텁 확장(V61), 발송 워커, EMAIL/INAPP 채널 실행기, PolicyDispatchController 권한 보강을 포함.
...
2. **`notification_template` 스키마 확장** — V61 마이그레이션 (additive ALTER).
...
7. **권한 시드 추가** — `NOTIFICATION_TEMPLATE:READ/WRITE/DELETE`, `DISPATCH:WRITE` (V61).
...
- **REQ-NE-024** (Ubiquitous): The V61 migration shall be additive (ALTER ADD COLUMN with defaults), preserving the existing `notification_template.id` FK relationship with `notification_dispatch_schedule.template_id`.
...
- **AC-NE-001**: V61 마이그레이션 적용 후 `notification_template` 테이블에 `subject`, `body_html`, `variables`, `language`, `is_active`, `email_template_id`, `created_by`, `updated_by`, `created_at`, `updated_at` 컬럼이 존재하고, 기존 `id` 컬럼과 `notification_dispatch_schedule.template_id` FK 제약이 유지된다. (REQ-NE-001, REQ-NE-024)
...
- **AC-NE-017**: V61 적용 후 `permissions` 테이블에 `NOTIFICATION_TEMPLATE:READ/WRITE/DELETE`, `DISPATCH:WRITE` 4개 코드가 존재하고 `SUPER_ADMIN`에 모두 매핑된다. 재적용(idempotent) 시 `ON CONFLICT DO NOTHING`으로 중복 없이 통과한다. (REQ-NE-020)
...
## Migration Plan (V61 DDL 개요)

다음 마이그레이션 버전은 **V61**이다. 단일 마이그레이션에 스키마 확장 + 권한 시드를 함께 담는다.

```sql
-- V61__notification_template_extension.sql  (SPEC-CMS-NOTI-EXT-001)
...
└── V61__notification_template_extension.sql              (신규)
```
...
```
Phase 1 (백엔드 기반)
  1. V61 마이그레이션 (notification_template 확장 + 권한 시드 + SUPER_ADMIN 매핑)
