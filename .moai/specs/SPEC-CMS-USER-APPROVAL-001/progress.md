...
| ID | 작업 | 상태 | 비고 |
|----|------|------|------|
| T0 | DB 마이그레이션 `V62__user_registration_approval.sql` (status 제약 재정의 + additive 3컬럼 + system_setting/권한/템플릿 시드) | DONE | 단일 파일 통합 (NFR-UA-C2) |
...
| T10 | 테스트 (백엔드 단위·통합, 프론트 컴포넌트) | DONE | `MigrationOrderIT` 카운트 61→62 갱신 포함 |
...
| 게이트 | 상태 | 비고 |
|--------|------|------|
| 마이그레이션 V62 적용 (제약 재정의 무손실) | DONE | `MigrationOrderIT` EXPECTED_MIGRATION_COUNT 62로 검증 |
