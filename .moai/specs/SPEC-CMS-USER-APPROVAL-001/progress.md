...
| ID | 작업 | 상태 | 비고 |
|----|------|------|------|
| T0 | DB 마이그레이션 `V59__user_registration_approval.sql` (status 제약 재정의 + additive 3컬럼 + system_setting/권한/템플릿 시드) | TODO | 단일 파일 통합 (NFR-UA-C2) |
...
| 게이트 | 상태 | 비고 |
|--------|------|------|
| 마이그레이션 V59 적용 (제약 재정의 무손실) | TODO | 기존 4개 상태 행 영향 없음 확인 |
