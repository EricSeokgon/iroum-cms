...
- **단일 마이그레이션**: 모든 스키마/시드를 `V60__points_system.sql` 하나로 묶는다.
...
### T1: DB 마이그레이션 V60
- 파일: `backend/src/main/resources/db/migration/V60__points_system.sql`
...
- **정책 미시드 환경**: 기존 운영 DB에 POINTS:* 키가 없을 때. → REQ-PNT-001 기본값 처리 + V60 seed로 보장.
- **권한 시드 누락**: POINTS:READ/WRITE 권한이 ADMIN 역할에 매핑되지 않으면 관리자 접근 불가. → V60에서 권한 seed 및 역할 매핑 확인.
