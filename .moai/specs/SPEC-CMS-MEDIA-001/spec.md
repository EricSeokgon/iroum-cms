...
- AV(Anti-Virus) 스캔 비동기 큐 (ClamAV 데몬 연동)
...
- 1차 저장소: Local FS — `MediaStorage` 인터페이스로 추상화, 2차 옵션 MinIO/S3
...
| MinIO/S3 1차 도입 | 인터페이스만 두고, 도입은 2차 결정 (사용자 협의 필요, 위험·완화 §12 참조) |
...
erDiagram
    ...
    media_processing_job {
        bigint id PK
        bigint asset_id FK
        varchar job_type "WEBP_CONVERT/THUMBNAIL/EXIF_STRIP/AV_SCAN"
...
CREATE TABLE media_processing_job (
    ...
    error_message TEXT,
    CONSTRAINT chk_job_type   CHECK (job_type IN ('WEBP_CONVERT','THUMBNAIL','EXIF_STRIP','AV_SCAN')),
    CONSTRAINT chk_job_status CHECK (status IN ('PENDING','RUNNING','SUCCESS','FAILED'))
);

COMMENT ON TABLE media_processing_job IS '비동기 후처리 작업 큐. PENDING 행을 워커가 폴링.';
```
...
- **REQ-MEDIA-002-D-4 (State-Driven, AV 스캔 비동기)**: WHILE 자산 status가 `PROCESSING`인 동안, THE system SHALL ClamAV 데몬에 스캔 작업을 비동기 큐잉하고, 스캔 결과 `INFECTED`인 경우 자산을 즉시 `DELETED`로 전이시키며 `stored_path` 파일을 격리(quarantine) 디렉터리로 이동한다.
...
- **AV 스캔 (REQ-MEDIA-002-D-4)**: ClamAV 데몬, INFECTED 시 즉시 격리 + 자산 DELETED.
...
| R-MEDIA-2 | 악성 파일 위장 (jpg 가장 PHP 등) | RCE | 매직넘버 검증 + AV 스캔 + webroot 외부 저장 + Content-Disposition: attachment |
...
| R-MEDIA-7 | LocalFS 단일 노드 SPOF | 가용성 저하 | NAS 또는 2차 MinIO/S3 도입 검토 — 사용자 협의 필요 |
| R-MEDIA-8 | ClamAV 데몬 다운 | 후처리 정체 | AV 스캔 작업은 RETRY 정책 (지수 백오프, 5회), 최종 실패 시 자산은 PROCESSING 유지 + 관리자 알림 |
