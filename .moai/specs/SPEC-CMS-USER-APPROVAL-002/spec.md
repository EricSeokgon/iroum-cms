...
- **NFR-UA2-C3**: 마이그레이션은 단일 Flyway 파일에 통합한다(additive 컬럼 + 설정 시드 + 이메일 템플릿 3종 시드). **마이그레이션 번호는 APPROVAL-001 머지 후 최신 번호의 다음 값**으로 run 직전 확정한다(잠정 V65).
...
| ID | 작업 | 우선순위 |
|----|------|---------|
| T0 | 마이그레이션(잠정 V65): `reminder_sent_at` 컬럼 + `email_verified_at` 컬럼 + 설정 3종 + 이메일 템플릿 2종 시드(`USER_APPROVAL_REMINDER`, `USER_APPROVAL_AUTO_REJECTED`) | High |
